...
function getShiftDef(shiftType, shiftDefs, dept) {
  ...
  return (dept && shiftDefs[`${dept}:${shiftType}`]) || shiftDefs[shiftType] || { label: shiftType, start: null, end: null, color: "#F5F9FE", tc: "#6b7280", breakMin: 0 };
}
            ...
            {disp.map(r => {
              ...
              return (
                <tr key={r.ds} style={{ borderBottom: "0.5px solid #e9ddd0", background: isToday ? "#EFF6FF" : edits[r.ds] ? "#FFF8E1" : r.needsConfirm || r.absent ? "#FFF8F8" : "inherit" }}>
                  ...
                  <td style={tdS}>
                    {editMode ? (
                      <input type="time" value={edits[r.ds]?.out ?? r.punch?.out ?? ""} onChange={e => onEdit(r.ds, "out", e.target.value)} style={iST} />
                    ) : (
                      <>
                        {adjOutStr}
