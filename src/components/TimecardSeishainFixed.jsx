...
export default function TimecardSeishainFixed({ emp, shifts, punches, shiftDefs, lvReqs, timeTransferReqs, isAdmin = false }) {
  ...
  const approvedTypeC = (timeTransferReqs || [])
    .filter(r => String(r.empId) === String(emp.id) && r.transferType === "C" && r.status === "approved" && periodWeekStarts.has(r.overWeekStart || ""))
  ...
  const statusBadge = r => {
    ...
    // 残業バッジ：バッジは常に表示・時間は管理者のみ
    if (r.otMin > 0) badges.push(
      <span key="ot" style={{ display: "inline-flex", alignItems: "center", gap: 2 }}>
        <Badge label="残業" bg="#FAEEDA" color="#854F0B" />
        {isAdmin && <span style={{ fontSize: 11, color: "#854F0B" }}>+{toHStr(r.otMin)}</span>}
      </span>
    );
      ...
      <div style={{ ...crd, padding: "12px 14px", marginBottom: "1rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8 }}>
          {[
            ...
            <div key={l} style={{ textAlign: "center", padding: "8px 4px", background: l === "要確認" && cd > 0 ? "#FFF0F0" : "#fef9f3", border: l === "要確認" && cd > 0 ? "0.5px solid #F09595" : "none", borderRadius: 8 }}>
