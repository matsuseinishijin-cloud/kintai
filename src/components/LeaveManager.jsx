            ...
            <tbody>
              {(lvReqs || []).filter(r => String(r.empId) === String(sel)).sort((a, b) => b.date > a.date ? 1 : -1).map(r => (
                <tr key={r.id} style={{ borderBottom: "0.5px solid #e9ddd0" }}>
                  ...
                  <td style={tdS}>{(r.reason === "指定休" || r.reason?.startsWith("指定有休")) ? <span style={{ padding: "2px 8px", borderRadius: 99, fontSize: 11, background: "#EDE9FE", color: "#7C3AED" }}>指定休</span> : isHalfLeave(r.half) ? "半日（0.5日）" : "全日（1日）"}</td>
