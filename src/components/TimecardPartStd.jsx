...
import { toMin, toHStr, pad, DOW_JP, getPeriodRange, getPeriodDays, today } from "../utils/time";
...
// 時間帯別集計
// ①午前：8:40〜14:00  ②午後：14:00〜18:00  ③夜勤：18:00〜  ④日曜
const AM_START = toMin("08:40"), AM_END = toMin("14:00");
const PM_START = toMin("14:00"), PM_END = toMin("18:00");
const NIGHT_START = toMin("18:00");

function calcTimeSlots(inMin, outMin, dow) {
  if (!inMin || !outMin || inMin >= outMin) return { am: 0, pm: 0, night: 0, sun: 0 };
  const isSun = dow === 0;

  if (isSun) {
    return { am: 0, pm: 0, night: 0, sun: Math.max(0, outMin - inMin) };
  }

  const am = Math.max(0, Math.min(outMin, AM_END) - Math.max(inMin, AM_START));
  const pm = Math.max(0, Math.min(outMin, PM_END) - Math.max(inMin, PM_START));
  const night = Math.max(0, outMin - Math.max(inMin, NIGHT_START));

  return { am: Math.max(0, am), pm: Math.max(0, pm), night: Math.max(0, night), sun: 0 };
}
...
function calcDay(ds, emp, shiftDefs, shifts, punches, lvReqs) {
  ...
  if (isLeave) {
    ...
    if (!isOff) {
      ...
      if (om < shiftE - 1) {
        earlyLeave = true;
        earlyLeaveMin = Math.floor((shiftE - om) / roundMin) * roundMin;
      }
...
export default function TimecardPartStd({ emp, shifts, punches, shiftDefs, lvReqs }) {
  const now = new Date();
  ...
  const totalPmMin = rows.reduce((s, r) => s + r.slots.pm, 0);
  const totalNightMin = rows.reduce((s, r) => s + r.slots.night, 0);
    ...
    <div>
      ...
      <div style={{ ...crd, padding: "12px 14px", marginBottom: "1rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8 }}>
          {[
            ["①午前（8:40〜14:00）", totalAmMin],
            ["②午後（14:00〜18:00）", totalPmMin],
            ["③夜勤（18:00〜）", totalNightMin],
            ["④日曜", totalSunMin],
          ].map(([l, v]) => (
