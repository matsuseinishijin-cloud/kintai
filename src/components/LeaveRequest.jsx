...
export default function LeaveRequest({ emp, leaves, lvReqs, shifts, shiftDefs, reload }) {
  ...
  approvedSorted.forEach(req => {
    ...
    let eligible = bucketsWithRem.filter(b => b.grantedAt <= req.date && (!b.expiresAt || b.expiresAt >= req.date) && b.remaining > 0);
    if (eligible.length === 0) {
      // 通常の紐づけ先がない場合、取得日より後に付与された一番早いバケツに繰り越し不足として紐づける
      const future = bucketsWithRem.filter(b => b.grantedAt > req.date).sort((a, b) => a.grantedAt > b.grantedAt ? 1 : -1);
                  ...
                  <div key={bucketId} style={{ marginBottom: 12, borderRadius: 8, border: `1px solid ${isExpired ? "#e9ddd0" : "#c7d2fe"}`, overflow: "hidden" }}>
                    ...
                    {b.assignedReqs.length === 0 ? (
                      ...
                      <div key={r.id} style={{ padding: "6px 12px", borderTop: "0.5px solid #e9ddd0", display: "flex", alignItems: "center", gap: 10, fontSize: 12 }}>
                        ...
                        <span style={{ color: "#374151" }}>{(r.reason === "指定休" || r.reason?.startsWith("指定有休")) ? "指定休" : isHalfLeave(r.half) ? "半日 0.5日" : "全日 1.0日"}</span>
                        {r.leaveStart && r.leaveEnd && <span style={{ color: "#6b7280" }}>{r.leaveStart}〜{r.leaveEnd}</span>}
                        {r.reason && r.reason !== "指定休" && !r.reason.startsWith("指定有休") && <span style={{ color: "#9ca3af", fontSize: 11 }}>（{r.reason}）</span>}
