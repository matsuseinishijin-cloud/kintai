...
const LV_REQ_MAP = { id:"id", "従業員id":"empId", "日付":"date", "理由":"reason", "状態":"status", "半日":"half", "有休開始":"leaveStart", "有休終了":"leaveEnd" };
...
const SHIFT_CONFIRM_INV=invertMap(SHIFT_CONFIRM_MAP);
const TIME_TRANSFER_INV=invertMap(TIME_TRANSFER_MAP);
...
// ── 理学療法士パート 照合ロジック（グローバル） ──────────────────────────────
// empId・ds・otReqsを受け取り、調整後の出退勤・実働を返す
function calcPTAdj({empId, ds, punch, def, otReqs=[]}){
  if(!punch||!def.start) return {inT:punch?.in||"",outT:punch?.out||"",workMin:0,lateMin:0,lateDeduct:0,otMin:0,isLate:false,isEarly:false,isOT:false,adjOutRaw:null};
  ...
  const approvedOT=otReqs.find(req=>String(req.empId)===String(empId)&&req.date===ds&&req.status==="approved"&&req.type==="overtime");
  const approvedEarly=otReqs.find(req=>String(req.empId)===String(empId)&&req.date===ds&&req.status==="approved"&&req.type==="early");
  // 退勤調整
  let adjOut=pOut;
  if(pOut!==null){
  ...
  return {inT:punch.in||"",outT:adjOut!==null?fmtTime(adjOut):"",workMin,lateMin,lateDeduct,otMin,isLate,isEarly,isOT:otMin>0,adjOutRaw:adjOut};
}
...
function buildRows(emp, shifts, punches, otReqs, lvReqs, year, month, shiftDefsData={}){
  ...
  return Array.from({length:last},(_,i)=>{
    ...
    const isLeave=!!_lvMatch;
    const leaveHalf=_lvMatch?.half||null;
    // シフトなし半日有休で出勤時刻が入力されている場合
    const halfLeaveStart=_lvMatch?.leaveStart||"";
    const halfLeaveEnd=_lvMatch?.leaveEnd||"";
    const isHalfLeaveWithTime=isLeave&&leaveHalf&&halfLeaveStart&&halfLeaveEnd;
    const isFullLeave=isLeave&&!leaveHalf;
...
function ShiftCalendar({emps,shifts:shiftsFromProps,shiftDefsData,reload,leadRoles:initLeadRoles=null,lvReqs=[],timeTransferReqs=[],onGotoShiftSetting=null,onGotoPattern=null}){
  ...
  const saveAll=async()=>{
    ...
    try{
      ...
      setLocalEdits({});
      await reload();
      // ※leave系シフト運用廃止のため、保存時の自動有休消化処理は一時無効化
      // （指定有休専用の新仕組みに置き換え予定）
      /*
      const td=today();
                ...
                return [
                  ...wk.map(ds=>{
                    ...
                    const lvBadge=lvReq?(lvReq.status==="approved"
                      ...
                      :{label:halfLabel,bg:"#A32D2D",tc:"#fff"})
                      :null;
                    ...
                    return <td key={ds} style={{padding:"2px",borderBottom:"0.5px solid var(--color-border-tertiary)",textAlign:"center",verticalAlign:"middle",cursor:isLocked?"not-allowed":clickable?"pointer":"default",userSelect:"none",borderLeft:dow===1?"2px solid #d1d5db":"none",background:isLocked?"#F0FAF5":isNextMonth?"#f5f5f5":isHol?"#FFF0F0":"inherit"}} onClick={()=>clickable&&!isLocked&&setCell(emp.id,ds)}>
                      <div style={{position:"relative",display:"inline-block",width:"100%"}}>
                        <div style={{background:isHalfLeave?"#E1F5EE":def.color,color:isHalfLeave?"#0F6E56":def.tc,borderRadius:4,padding:"2px 3px",fontSize:isCustom?7:isNextMonth?11:15,minWidth:isCustom?56:isNextMonth?36:48,border:"1px solid transparent",fontWeight:400,textAlign:"center",whiteSpace:"nowrap",opacity:isNextMonth?0.5:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:isCustom?36:undefined}}>
                          {isHalfLeave?halfLeaveLabel:isCustom&&customParsed?<><div style={{fontSize:9,lineHeight:1.4,whiteSpace:"nowrap",fontWeight:500}}>{customParsed.start}</div><div style={{fontSize:9,lineHeight:1.4,whiteSpace:"nowrap",fontWeight:500}}>{customParsed.end}</div></>:def.label}
                        </div>
                        {lvBadge&&<div
...
function LeaveManager({emps,leaves,lvReqs,shifts=[],shiftDefsData={},reload,canGrant=true}){
  const cur0=getCurrentPeriod();
  ...
  const decide=async(id,status)=>{
    ...
    try{
      if(status==="approved"){
        ...
        const hasShift=!!def.start;
        if(hasShift){
          if(!req.half){
            // 全日有休：シフトが入っていたら承認不可
            alert(`${req.date} は既にシフトが入っているため承認できません。先にシフトを休日に変更してください。`);
            return;
          } else {
            // 半日有休：申請した有休時間帯とシフト時間帯が重なったら承認不可
            if(req.leaveStart&&req.leaveEnd){
              const shiftStart=toMin(def.start),shiftEnd=toMin(def.end);
              const lvStart=toMin(req.leaveStart),lvEnd=toMin(req.leaveEnd);
              const overlaps=lvStart<shiftEnd&&lvEnd>shiftStart;
              if(overlaps){
                alert(`${req.date} の有休時間帯（${req.leaveStart}〜${req.leaveEnd}）がシフト（${def.start}〜${def.end}）と重なっています。先にシフトを調整してください。`);
                return;
              }
            }
          }
      ...
      if(status==="approved"&&req.date>td){
        ...
        alert(`${req.date} は未来の日付のため承認しました。`);
      } else {
        // 過去・当日 → 即時消化
  ...
  // 承認済み有休の取消 → rejected化（シフトには手を加えない）
...
function TimecardView({emps,shifts,punches,otReqs,lvReqs,shiftDefsData,isAdmin=false,leadRoles=null,selfView=false,reload=()=>{},onGotoShiftCalendar=null,timeTransferReqs=[],shiftConfirmReqs=[]}){
  const cur0=getCurrentPeriod();
    ...
    for(const e of targetEmps){
      ...
      for(const [y,m] of [[prevY,prevMM],[curY,curM]]){
        ...
        for(let d=1;d<=last;d++){
          ...
          const isLeave=!!_lv;
          if(isLeave) continue;
          // leave系シフト廃止：有休はlvReqsで判定済みのためスキップ不要
  ...
  const getPTAdjusted=(r)=>calcPTAdj({empId:emp?.id,ds:r.ds,punch:r.punch,def:r.def,otReqs:otReqs||[]});
  ...
  const rows=(()=>{
    ...
    return periodDays.map(ds=>{
      ...
      const halfLeaveEnd=_lvMatch?.leaveEnd||"";
      const isHalfLeaveWithTime=isLeave&&leaveHalf&&halfLeaveStart&&halfLeaveEnd;
      const isFullLeave=isLeave&&!leaveHalf; // 全日有休：所定・実働とも0
      const approvedEarlyReq=(otReqs||[]).find(r=>String(r.empId)===String(emp.id)&&r.date===ds&&r.status==="approved"&&r.type==="early");
      const approvedOTReq=(otReqs||[]).find(r=>String(r.empId)===String(emp.id)&&r.date===ds&&r.status==="approved"&&r.type==="overtime");
      let swMin=0,awMin=0,otMin=0,diffMin=0,late=false,earlyLeave=false,earlyLeaveMin=0,absent=false;
      let adj=punch?.adjusted||false,earlyAdj=false;
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",marginBottom:"1rem"}}>
      ...
      {(isAdmin||(leadRoles&&leadRoles.includes("理学療法士")))&&emp&&!bulkMode&&<button onClick={startBulk} style={{...bP,padding:"8px 16px",fontSize:13}}>一括入力</button>}
      {(isAdmin||(leadRoles&&leadRoles.includes("理学療法士")))&&emp&&bulkMode&&<div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
    ...
    {/* 一括入力モード */}
    {(isAdmin||(leadRoles&&leadRoles.includes("理学療法士")))&&bulkMode&&emp&&<div style={{...crd,overflow:"hidden",marginBottom:"1rem"}}>
    ...
    {subTabs.length>0&&<div style={{display:"flex",gap:4,marginBottom:"1rem",borderBottom:"2px solid var(--color-border-tertiary)"}}>
      {subTabs.map((t,i)=><button key={t} onClick={()=>setSubTab(i)} style={{padding:"8px 18px",border:"none",borderBottom:subTab===i?"2px solid #1251a3":"2px solid transparent",background:"transparent",color:subTab===i?"#1251a3":"var(--color-text-secondary)",fontWeight:subTab===i?600:400,fontSize:13,cursor:"pointer",marginBottom:"-2px",display:"flex",alignItems:"center",gap:4}}>
        {t}
        {t==="要確認"&&offPunchItems.length>0&&<span style={{padding:"1px 6px",borderRadius:99,fontSize:10,background:"#5B21B6",color:"#fff",fontWeight:700}}>{offPunchItems.length}</span>}
      </button>)}
    </div>}
    ...
    {!isPTpart&&subTabs[subTab]==="月次レポート"&&emp&&(()=>{
      ...
      return <div>
        ...
        <ReportView emps={[emp]} shifts={shifts} punches={punches} otReqs={otReqs} lvReqs={lvReqs} initEmpId={emp.id} shiftDefsData={shiftDefsData} isAdmin={!selfView} selfView={selfView} leadRoles={leadRoles} reload={reload} outerYear={year} outerMonth={month} timeTransferReqs={timeTransferReqs} shiftConfirmReqs={shiftConfirmReqs}/>
      </div>;
    })()}
    ...
    {subTab===subTabs.length-1&&!selfView&&<div>
      <div style={{fontSize:13,fontWeight:700,marginBottom:"1rem",color:"#111"}}>
        シフトなし打刻一覧
        <span style={{marginLeft:8,fontSize:11,fontWeight:400,color:"var(--color-text-secondary)"}}>当月・先月</span>
      </div>
      {offPunchItems.length===0
        ...
        :<div style={{...crd,overflow:"hidden"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
            ...
            <tbody>{offPunchItems.map((item,i)=>{
              ...
              return <tr key={i} style={{borderBottom:"0.5px solid var(--color-border-tertiary)",background:"#F5F3FF"}}>
                <td style={tdS}><div style={{display:"flex",alignItems:"center",gap:6}}>
                  <div style={{width:22,height:22,borderRadius:"50%",background:AVATAR_COLORS[emps.findIndex(e=>e.id===item.emp.id)%AVATAR_COLORS.length][0],color:AVATAR_COLORS[emps.findIndex(e=>e.id===item.emp.id)%AVATAR_COLORS.length][1],display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:500}}>{item.emp.name[0]}</div>
...
// ── ReportView ────────────────────────────────────────────────────────────────
function ReportView({emps,shifts,punches,otReqs,lvReqs,initEmpId,shiftDefsData,isAdmin=false,leadRoles=null,selfView=false,reload=()=>{},outerYear=null,outerMonth=null,timeTransferReqs=[],shiftConfirmReqs=[]}){
  const cur0=getCurrentPeriod();
  ...
  const overtimeMin=(()=>{
    ...
    periodDays.forEach(ds=>{
      ...
      // 有休はlvReqsベースで加算（leave系シフト廃止）
      const lvA=(lvReqs||[]).find(r=>String(r.empId)===String(emp.id)&&r.date===ds&&r.status==="approved");
      if(lvA){
        const lvMin=lvA.leaveStart&&lvA.leaveEnd?Math.max(0,toMin(lvA.leaveEnd)-toMin(lvA.leaveStart)):(lvA.half?240:480);
        weekMap[wk]+=lvMin;
      } else {
        const def=empDefs[st]||empDefs.off||SHIFT_DEFS.off;
    ...
    // 週ごとの超過分を合算
    let total=Object.entries(weekMap).reduce((s,[wk,wMin])=>s+Math.max(0,wMin-wLimit),0);
    // 承認済み時間振替申請（タイプA）の相殺分を差し引く
    const approvedA=(timeTransferReqs||[]).filter(r=>String(r.empId)===String(emp.id)&&r.status==="approved"&&(r.transferType==="A"||!r.transferType));
    approvedA.forEach(r=>{
      const overWeekDs=periodDays.find(ds=>{
        const d=new Date(ds);
        const dow=d.getDay();
        const diff=dow===0?-6:1-dow;
        const mon=new Date(d);mon.setDate(d.getDate()+diff);
        return mon.toISOString().slice(0,10)===r.overWeekStart;
      });
      if(overWeekDs) total-=Number(r.offsetMin||0);
    });
    // 承認済み時間外申請（タイプC）は時間外として計上済みなので超過から除外
    const approvedC=(timeTransferReqs||[]).filter(r=>String(r.empId)===String(emp.id)&&r.status==="approved"&&r.transferType==="C");
    approvedC.forEach(r=>{
      const overWeekDs=periodDays.find(ds=>{
        const d=new Date(ds);
        const dow=d.getDay();
  ...
  return <div>
    ...
    {emp&&<div style={{...crd,padding:"12px 14px",marginBottom:"1rem"}}>
      ...
      {(()=>{
        ...
        // その他職種：従来のグリッド＋時間外セル
        return <>
          <div style={{display:"grid",gridTemplateColumns:`repeat(${showOvertimeCell?8:7},1fr)`,gap:8}}>
            ...
            {showOvertimeCell&&<div style={{textAlign:"center",padding:"8px 4px",background:totalOvertimeMin>0?"#FFF8E1":"var(--color-background-secondary)",border:totalOvertimeMin>0?"0.5px solid #F59E0B":"none",borderRadius:8}}>
              <div style={{fontSize:10,color:"var(--color-text-secondary)",marginBottom:2}}>時間外</div>
              <div style={{fontSize:16,fontWeight:500,color:totalOvertimeMin>0?"#854F0B":"var(--color-text-primary)"}}>{totalOvertimeMin>0?toHStr(totalOvertimeMin):"―"}</div>
            </div>}
    ...
    {emp&&<div style={{...crd,overflow:"hidden"}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
...
function PunchScreen({emp,punches:punchesFromProps,shifts,shiftDefsData,reload,onPunchesUpdate=()=>{},onGotoRequest=null,lvReqs=[],leaves=[],otReqs=[],punchFixReqs=[],timeTransferReqs=[]}){
  ...
  const pr=getPeriodRange(period.year,period.month);

  // ①シフト超過・不足がある週
  if(emp.weeklyLimit){
    const weekLimit=Number(emp.weeklyLimit)*60;
    const seen=new Set();
    const [sy,sm,sd]=pr.start.split("-").map(Number);
    const [ey,em,ed]=pr.end.split("-").map(Number);
    let cur=new Date(sy,sm-1,sd);
    const endD=new Date(ey,em-1,ed);
    while(cur<=endD){
      const ds=`${cur.getFullYear()}-${pad(cur.getMonth()+1)}-${pad(cur.getDate())}`;
      const dow=cur.getDay();const diff=dow===0?-6:1-dow;
      const mon=new Date(cur);mon.setDate(cur.getDate()+diff);
      ...
      if(!seen.has(monStr)){
        ...
        for(let i=0;i<7;i++){
          ...
          const st2=sr?.shiftType||"off";
          const def2=getShiftDefsByRole(emp.role,shiftDefsData||{})[st2];
          if(def2&&def2.start&&def2.end){const bk=def2.breakMin!=null?def2.breakMin:BREAK_MIN;wMin+=Math.max(0,toMin(def2.end)-toMin(def2.start)-bk);}
          const lv2=(lvReqs||[]).find(r=>String(r.empId)===String(emp.id)&&r.date===ds2&&r.status==="approved");
          if(lv2){wMin+=lv2.leaveStart&&lv2.leaveEnd?Math.max(0,toMin(lv2.leaveEnd)-toMin(lv2.leaveStart)):(lv2.half?240:480);}
        }
  ...
  // ②タイムカード要確認（当月内で打刻なしのシフトあり日）
  const [psy,psm,psd]=pr.start.split("-").map(Number);
  const [pey,pem,ped]=pr.end.split("-").map(Number);
  let confirmCount=0;
  let cur2=new Date(psy,psm-1,psd);const endD2=new Date(pey,pem-1,ped);
  while(cur2<=endD2){
    const ds2=`${cur2.getFullYear()}-${pad(cur2.getMonth()+1)}-${pad(cur2.getDate())}`;
    if(ds2<=td){
      const sr=shifts.find(s=>String(s.empId)===String(emp.id)&&s.date===ds2);
      const st2=sr?.shiftType||"off";
      const def2=getShiftDefsByRole(emp.role,shiftDefsData||{})[st2];
      const p2=punches.find(p=>String(p.empId)===String(emp.id)&&p.date===ds2);
      if(def2?.start&&!p2) confirmCount++;
      if(p2?.in&&!p2?.out) confirmCount++; // 退勤忘れ
    }
  ...
  if(punch?.in&&!punch?.out&&td>td) notifications.push({type:"warn",msg:"退勤打刻を忘れています"});
  ...
  // ⑤時間外申請が必要な週（理学療法士正社員・週所定40h未満）
  if(emp.role==="理学療法士"&&emp.type==="正社員"&&emp.weeklyLimit&&Number(emp.weeklyLimit)<40){
  ...
  const submitFix=async()=>{
    ...
    try{
      const cur3=punches.find(p=>String(p.empId)===String(emp.id)&&p.date===fixForm.date);
      const data=convertTo({id:newId(),empId:emp.id,date:fixForm.date,reqIn:fixForm.reqIn,reqOut:fixForm.reqOut,reason:fixForm.reason,status:"pending",origIn:cur3?.in||"",origOut:cur3?.out||""},PUNCH_FIX_INV);
      await gasSave("打刻修正申請",data);
      setFixForm({date:today(),reqIn:"",reqOut:"",reason:""});
      setFixSub(true);setTimeout(()=>setFixSub(false),3000);
      await reload();
  ...
  return <div style={{maxWidth:420}}>
    {/* 通知エリア */}
    {notifications.length>0&&<div style={{marginBottom:"1rem",display:"flex",flexDirection:"column",gap:6}}>
      {notifications.map((n,i)=>(
        <div key={i} style={{padding:"8px 12px",borderRadius:8,fontSize:12,fontWeight:500,
          background:n.type==="error"?"#FFF0F0":n.type==="warn"?"#FFF8E1":"#F0F4FF",
          color:n.type==="error"?"#A32D2D":n.type==="warn"?"#854F0B":"#1251a3",
          border:`1px solid ${n.type==="error"?"#F09595":n.type==="warn"?"#F59E0B":"#93C5FD"}`}}>
          {n.type==="error"?"🔴":n.type==="warn"?"⚠️":"ℹ️"} {n.msg}
        </div>
      ))}
    </div>}
    ...
    <div style={{...crd,padding:"1.25rem"}}>
      <div style={{fontSize:14,fontWeight:700,marginBottom:"1rem"}}>打刻修正申請</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
        <div><div style={{fontSize:11,color:"var(--color-text-secondary)",marginBottom:3}}>申請出勤時刻</div>
          <input type="time" value={fixForm.reqIn} onChange={e=>setFixForm(p=>({...p,reqIn:e.target.value}))} style={iS}/></div>
        <div><div style={{fontSize:11,color:"var(--color-text-secondary)",marginBottom:3}}>申請退勤時刻</div>
          <input type="time" value={fixForm.reqOut} onChange={e=>setFixForm(p=>({...p,reqOut:e.target.value}))} style={iS}/></div>
      </div>
      ...
      <button onClick={submitFix} disabled={!fixForm.reqIn||!fixForm.reqOut||!fixForm.reason} style={{...bP,width:"100%",padding:"10px 0",fontSize:14,opacity:(!fixForm.reqIn||!fixForm.reqOut||!fixForm.reason)?0.4:1}}>申請する</button>
      {fixSub&&<div style={{marginTop:8,fontSize:12,color:"#3B6D11",padding:"6px 10px",background:"#EAF3DE",borderRadius:6}}>申請しました。</div>}
    </div>
  </div>;
}
...
function LeaveRequest({emp,leaves,lvReqs,shifts=[],shiftDefsData={},reload,initDate=null,onClearInitDate=()=>{}}){
  ...
  const [form,setForm]=useState({date:initDate||"",half:"full",reason:"",leaveStart:"",leaveEnd:""}),[sub,setSub]=useState(false);
  ...
  const submit=async()=>{
    ...
    try{
      ...
      setForm({date:"",half:"full",reason:"",leaveStart:"",leaveEnd:""});setSub(true);setTimeout(()=>setSub(false),3000);
      await reload();
    }catch(e){alert("申請失敗："+e.message);}
  };
...
function ApprovalCenter({emps,otReqs,lvReqs,transferReqs,punchFixReqs,punches=[],shifts,shiftDefsData,leaves,reload,showOT=false,showPunchFix=true,showLeave=true,showEarly=true,shiftConfirmReqs=[],timeTransferReqs=[],otherReqs=[]}){
  ...
  const decideLv=async(id,status)=>{
    ...
    try{
      ...
      // 承認時：シフトとの重なりチェック（シフトは変更しない方針）
      if(status==="approved"){
        const empObj=emps.find(e=>String(e.id)===String(req.empId));
        const shiftRow=shifts.find(s=>String(s.empId)===String(req.empId)&&s.date===req.date);
        const empDefs=getShiftDefsByRole(empObj?.role||"",shiftDefsData||{});
        const def=empDefs[shiftRow?.shiftType||"off"]||empDefs.off||SHIFT_DEFS.off;
        const hasShift=!!def.start; // 休日（off）以外は勤務シフトあり
        if(hasShift){
          if(!req.half){
            // 全日有休：シフトが入っている＝重なるので承認不可
            alert(`${req.date} は既にシフトが入っているため承認できません。先にシフトを休日に変更してください。`);
            return;
          } else {
            // 半日有休：申請した有休時間帯とシフト時間帯が重なったら承認不可
    ...
    try{
      ...
      if(status==="approved"&&!isOTCase){
        ...
        const matchKey=Object.keys(empDefs).find(k=>{
          const d=empDefs[k];
          return d.start===req.startTime&&d.end===req.endTime;
        });
  ...
  // 時間振替申請の承認処理
  const decideTT=async(id,status)=>{
    const req=(timeTransferReqs||[]).find(r=>r.id===id);if(!req)return;
    try{
      await gasSave("時間振替申請",convertTo({...req,status},TIME_TRANSFER_INV));
      await reload();
    }catch(e){alert("更新失敗："+e.message);}
  };
        ...
        :<table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
          ...
          <tbody>{filtered.map((r,i)=>{
            ...
            return <tr key={r.id+r._type} style={{borderBottom:"0.5px solid var(--color-border-tertiary)"}}>
              ...
              <td style={tdS}>
                <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                  {isPending&&r._type!=="shiftconfirm"&&r._type!=="timetransfer"&&<>
                    <button onClick={()=>{
                  ...
                  {isPending&&r._type==="timetransfer"&&<>
                    <button onClick={()=>decideTT(r.id,"approved")} style={{padding:"3px 10px",borderRadius:6,background:"#EAF3DE",color:"#3B6D11",border:"none",fontSize:11,cursor:"pointer",fontWeight:500}}>承認</button>
                  ...
                  {isPending&&r._type==="shiftconfirm"&&(()=>{
                    ...
                    return <div style={{display:"flex",gap:4,alignItems:"center",flexWrap:"wrap"}}>
                      <select value={scApprove[r.id]||""} onChange={e=>setScApprove(p=>({...p,[r.id]:e.target.value}))}
                        style={{...iS,width:100,padding:"3px 6px",fontSize:11}}>
                        <option value="">シフト選択</option>
                        {shiftKeys.map(k=><option key={k} value={k}>{empDefs[k].label}（{empDefs[k].start}〜{empDefs[k].end}）</option>)}
                      </select>
                      <button onClick={()=>decideShiftConfirm(r.id,"approved",scApprove[r.id])}
                        disabled={!scApprove[r.id]}
                        style={{padding:"3px 10px",borderRadius:6,background:"#EAF3DE",color:"#3B6D11",border:"none",fontSize:11,cursor:scApprove[r.id]?"pointer":"default",fontWeight:500,opacity:scApprove[r.id]?1:0.4}}>承認</button>
                      <button onClick={()=>decideShiftConfirm(r.id,"rejected","")}
                        style={{padding:"3px 10px",borderRadius:6,background:"#FCEBEB",color:"#A32D2D",border:"none",fontSize:11,cursor:"pointer",fontWeight:500}}>却下</button>
                    </div>;
                  })()}
...
function OvertimeRequest({emp,shifts,shiftDefsData,timeTransferReqs=[],lvReqs=[],reload}){
  ...
  const getWeekShiftMin=(weekStart)=>{
    ...
    for(let i=0;i<7;i++){
      ...
      // 有休はlvReqsベースで加算（leave系シフト廃止）
      const lvA=(lvReqs||[]).find(r=>String(r.empId)===String(emp.id)&&r.date===ds&&r.status==="approved");
      if(lvA){ total+=lvA.leaveStart&&lvA.leaveEnd?Math.max(0,toMin(lvA.leaveEnd)-toMin(lvA.leaveStart)):(lvA.half?240:480); continue; }
      ...
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
        ...
        <tbody>{myReqs.map(r=>(
          <tr key={r.id} style={{borderBottom:"0.5px solid var(--color-border-tertiary)"}}>
            ...
            <td style={{...tdS,fontWeight:600,color:"#1251a3"}}>{toHStr(Number(r.offsetMin||0))}</td>
            <td style={tdS}>{r.status==="pending"?<Badge label="承認待ち" bg="#FAEEDA" color="#854F0B"/>:r.status==="approved"?<Badge label="承認済" bg="#EAF3DE" color="#3B6D11"/>:<Badge label="却下" bg="#FCEBEB" color="#A32D2D"/>}</td>
          </tr>
        ))}</tbody>
      </table>
  ...
  const weekOptions=(()=>{
    ...
    for(let mo=-1;mo<=0;mo++){
      ...
      for(let d=1;d<=last;d++){
        const ds=`${y}-${pad(m)}-${pad(d)}`;
        const mon=getMondayOf(ds);
        if(!seen.has(mon)){seen.add(mon);opts.push(mon);}
      }
    }
        ...
        <div style={{marginBottom:10}}>
          ...
          <select value={form.overWeekStart} onChange={e=>setForm(p=>({...p,overWeekStart:e.target.value,offsetMin:""}))} style={iS}>
            ...
            {weekOptions.map(w=>{
              ...
              // タイプA（振替）またはタイプC（時間外）承認済みを除外
              const alreadyUsed=(timeTransferReqs||[]).filter(r=>String(r.empId)===String(emp.id)&&r.overWeekStart===w&&r.status==="approved"&&(r.transferType==="A"||r.transferType==="C")).reduce((s,r)=>s+Number(r.offsetMin||0),0);
              // タイプC申請中は選択不可
              const hasPendingC=(timeTransferReqs||[]).some(r=>String(r.empId)===String(emp.id)&&r.overWeekStart===w&&r.transferType==="C"&&r.status==="pending");
              const remaining=excess-alreadyUsed;
              return <option key={w} value={w} disabled={remaining<=0||w===form.shortWeekStart||hasPendingC}>{w}週（シフト{toHStr(wMin)}{excess>0?`・超過${toHStr(remaining)}`:``}{hasPendingC?" ※時間外申請中":""}）</option>;
            })}
          </select>
...
function ShiftConfirmRequest({emp,punches,shifts,shiftConfirmReqs,shiftDefsData,reload}){
  const [form,setForm]=useState({date:"",reason:"",reasonDetail:"",startTime:"",endTime:"",breakMin:""});
  const [sub,setSub]=useState(false),[err,setErr]=useState("");

  // シフトなし打刻の日のみ選択可（当月・先月）
  const offPunchDates=(()=>{
    const td=today();
    const curY=new Date().getFullYear(),curM=new Date().getMonth()+1;
    const prevY=curM===1?curY-1:curY,prevMM=curM===1?12:curM-1;
    ...
    for(const [y,m] of [[prevY,prevMM],[curY,curM]]){
      ...
      for(let d=1;d<=last;d++){
        ...
        const st=shiftRow?.shiftType||"off";
        const def=empDefs[st]||empDefs.off||SHIFT_DEFS.off;
        if(!def.start) dates.push(ds);
      }
    }
    return dates;
  })();

  const isTransfer=form.reason==="振替出勤";
  const canSubmit=form.date&&form.reason&&!isTransfer&&form.startTime&&form.endTime
    &&(form.reason!=="その他"||form.reasonDetail.trim());
  ...
  const submit=async()=>{
    ...
    try{
      const data=convertTo({
        ...
        breakMin:form.breakMin?Number(form.breakMin):0,
        approvedShift:"",status:"pending"
      },SHIFT_CONFIRM_INV);
      await gasSave("シフト確認申請",data);
      setForm({date:"",reason:"",reasonDetail:"",startTime:"",endTime:"",breakMin:""});
      setSub(true);setTimeout(()=>setSub(false),3000);
      await reload();
    }catch(e){setErr("申請失敗："+e.message);}
  };

  const myReqs=(shiftConfirmReqs||[]).filter(r=>String(r.empId)===String(emp.id)).sort((a,b)=>b.date<a.date?-1:1);

  return <div>
    <div style={{...crd,padding:"1.25rem",marginBottom:"1rem",maxWidth:440}}>
      <div style={{fontSize:14,fontWeight:700,marginBottom:"1rem"}}>シフト確認申請</div>

      {offPunchDates.length===0
        ?<div style={{padding:"1rem",background:"var(--color-background-secondary)",borderRadius:8,fontSize:12,color:"var(--color-text-secondary)"}}>
          シフトなし打刻の日はありません
        </div>
        :<>
          {/* 日付 */}
          <div style={{marginBottom:10}}>
            <div style={{fontSize:11,color:"var(--color-text-secondary)",marginBottom:3}}>対象日</div>
            <select value={form.date} onChange={e=>setForm(p=>({...p,date:e.target.value}))} style={iS}>
              <option value="">選択してください</option>
          ...
          <div style={{marginBottom:10}}>
            <div style={{fontSize:11,color:"var(--color-text-secondary)",marginBottom:3}}>理由</div>
            <select value={form.reason} onChange={e=>setForm(p=>({...p,reason:e.target.value,reasonDetail:""}))} style={iS}>
              <option value="">選択してください</option>
          ...
          {/* 振替出勤の案内 */}
          {isTransfer&&<div style={{marginBottom:10,padding:"10px 12px",background:"#EAF3DE",borderRadius:8,fontSize:12,color:"#3B6D11"}}>
            振替出勤の場合は「申請」タブの<strong>振替申請</strong>から申請してください。
          </div>}

          {/* その他：理由詳細 */}
          {form.reason==="その他"&&<div style={{marginBottom:10}}>
            <div style={{fontSize:11,color:"var(--color-text-secondary)",marginBottom:3}}>理由詳細（必須）</div>
            <textarea value={form.reasonDetail} onChange={e=>setForm(p=>({...p,reasonDetail:e.target.value}))} rows={2} placeholder="理由を入力してください" style={{...iS,resize:"vertical"}}/>
          </div>}

          {/* シフト時刻入力 */}
          {!isTransfer&&form.reason&&<>
            <div style={{marginBottom:6,padding:"8px 12px",background:"#F5F9FE",borderRadius:8,fontSize:12,color:"#1251a3"}}>
              シフトの開始時刻・終了時刻を入力してください
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
              <div>
                <div style={{fontSize:11,color:"var(--color-text-secondary)",marginBottom:3}}>開始時刻</div>
                <input type="time" value={form.startTime} onChange={e=>setForm(p=>({...p,startTime:e.target.value}))} style={iS}/>
              </div>
              <div>
                <div style={{fontSize:11,color:"var(--color-text-secondary)",marginBottom:3}}>終了時刻</div>
                <input type="time" value={form.endTime} onChange={e=>setForm(p=>({...p,endTime:e.target.value}))} style={iS}/>
              </div>
            </div>
            <div style={{marginBottom:12}}>
              <div style={{fontSize:11,color:"var(--color-text-secondary)",marginBottom:3}}>休憩時間（分）　※なければ空欄</div>
              <input type="number" min={0} max={240} value={form.breakMin} onChange={e=>setForm(p=>({...p,breakMin:e.target.value}))} placeholder="例：60" style={iS}/>
            </div>
          </>}
    ...
    {myReqs.length>0&&<div style={{...crd,overflow:"hidden"}}>
      <div style={{padding:"10px 14px",borderBottom:"0.5px solid var(--color-border-tertiary)",fontSize:13,fontWeight:500}}>申請履歴</div>
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
        <thead><tr>{["日付","理由","シフト時刻","休憩","承認シフト","状態"].map(h=><th key={h} style={thS}>{h}</th>)}</tr></thead>
        <tbody>{myReqs.map(r=>(
          <tr key={r.id} style={{borderBottom:"0.5px solid var(--color-border-tertiary)"}}>
            <td style={tdS}>{r.date}</td>
            <td style={{...tdS,color:"var(--color-text-secondary)"}}>{r.reason}{r.reasonDetail?`（${r.reasonDetail}）`:""}</td>
            <td style={tdS}>{r.startTime}〜{r.endTime}</td>
            <td style={tdS}>{r.breakMin?r.breakMin+"分":"―"}</td>
            <td style={{...tdS,fontWeight:500}}>{r.approvedShift||"―"}</td>
            <td style={tdS}>{r.status==="pending"?<Badge label="承認待ち" bg="#FAEEDA" color="#854F0B"/>:r.status==="approved"?<Badge label="承認済" bg="#EAF3DE" color="#3B6D11"/>:<Badge label="却下" bg="#FCEBEB" color="#A32D2D"/>}</td>
          </tr>
        ))}</tbody>
      </table>
...
function RequestTab({emp,leaves,lvReqs,shifts,otReqs,punches,punchFixReqs,shiftDefsData,transferReqs,shiftConfirmReqs,timeTransferReqs,otherReqs,reload,initLeaveDate=null,onClearInitLeaveDate=()=>{}}){
  ...
  const sections=isOvertimeRequestTarget?[
    ...
    {key:"shiftconfirm",label:"シフト確認申請"},
  ];
  const firstKey=sections[0]?.key||"punchfix";
  // initLeaveDateがある場合は有給申請セクションを自動表示
  const [section,setSection]=useState(initLeaveDate&&hasLeave?"leave":firstKey);
  useEffect(()=>{
    if(initLeaveDate&&hasLeave){ setSection("leave"); }
  },[initLeaveDate]);
...
function MyShiftCalendar({emp,shifts,lvReqs,shiftDefsData,punches=[],otReqs=[],year,month}){
  ...
  return <div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:4}}>{cells.map((d,i)=>{
      ...
      const isLeave=!!_lvMatch,leaveHalf=_lvMatch?.half||null;
      const halfWorkIn=_lvMatch?.leaveStart||"";
      const halfWorkOut=_lvMatch?.leaveEnd||"";
      const shiftRow=shifts.find(s=>String(s.empId)===String(emp.id)&&s.date===ds);
      ...
      const _lvAny=_lvMatch||_lvPending;
      const lvHalfLabel=_lvAny?.half==="am"?"午前":_lvAny?.half==="pm"?"午後":"1日";
      // 有給バッジ：承認済→緑、未承認（申請中）→赤、申請なし→バッジなし
      const lvStatusBadge=_lvAny?(_lvMatch
        ?{label:lvHalfLabel,bg:"#0F6E56",tc:"#fff"}
        :{label:lvHalfLabel,bg:"#A32D2D",tc:"#fff"})
        :null;
      ...
      return <div key={i} style={{borderRadius:8,padding:"6px 8px",background:cellBg,border:cellBorder,minHeight:72}}>
        ...
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:4}}>
          ...
          {lvStatusBadge&&<div style={{marginTop:2}}><span style={{fontSize:9,fontWeight:700,padding:"1px 5px",borderRadius:3,background:lvStatusBadge.bg,color:lvStatusBadge.tc,whiteSpace:"nowrap"}}>{lvStatusBadge.label}</span></div>}
        </div>
      </div>;
    })}</div>
  </div>;
...
export default function App(){
  ...
  const [initLeaveDate,setInitLeaveDate]=useState(null);
  ...
  // ※leave系シフト運用廃止のため一時無効化（指定有休専用の新仕組みに置き換え予定）
  // useEffect(()=>{
  ...
  const offPunchCount=(()=>{
    ...
    const curY=new Date().getFullYear(),curM=new Date().getMonth()+1;
    const prevY=curM===1?curY-1:curY,prevMM=curM===1?12:curM-1;
    const targetEmps=isAdmin?emps:isLead?emps.filter(e=>leadRolesList.includes(e.role)):[];
    let count=0;
    for(const e of targetEmps){
      const empDefs=getShiftDefsByRole(e.role,shiftDefsData||{});
      for(const [y,m] of [[prevY,prevMM],[curY,curM]]){
  ...
  return <div style={{fontFamily:"var(--font-sans)",padding:"0 0 2rem"}}>
          ...
          <div style={{display:"flex",gap:3,padding:"4px 6px",borderRadius:10,background:"#ffffff",alignItems:"center",height:"100%"}}>
            {tabs.slice(tabs.indexOf("---")+1).map((t,i)=>{
              ...
              return <button key={t+i} onClick={()=>{if(shiftDefSaving)return;setTabName(t);setShowPwChange(false);}} style={{flex:1,padding:"8px 4px",borderRadius:8,border:"none",background:isActive?"#185FA5":"transparent",color:isActive?"white":"#185FA5",fontSize:14,fontWeight:isActive?700:500,cursor:shiftDefSaving?"not-allowed":"pointer",whiteSpace:"nowrap",opacity:shiftDefSaving&&!isActive?0.4:1,transition:"background 0.15s",textAlign:"center"}}>
                ...
                {t==="タイムカード"&&offPunchCount>0&&<span style={{marginLeft:4,padding:"1px 5px",borderRadius:99,fontSize:9,background:"#5B21B6",color:"white"}}>{offPunchCount}</span>}
              </button>;
            })}
          </div>
        </div>
    ...
    <div>
      ...
      {!isAdmin&&cur&&(()=>{
        ...
        if(t==="打刻") return <PunchScreen emp={cur} punches={punches} shifts={shifts} shiftDefsData={shiftDefsData} reload={loadAll} onPunchesUpdate={setPunches} lvReqs={lvReqs} leaves={leaves} otReqs={otReqs} punchFixReqs={punchFixReqs} timeTransferReqs={timeTransferReqs} onGotoRequest={(date)=>{setInitLeaveDate(date);setTabName("申請");}}/>;
        ...
        if(t==="タイムカード"&&isPTSeishain) return <TimecardView emps={[cur]} shifts={shifts} punches={punches} otReqs={otReqs} lvReqs={lvReqs} shiftDefsData={shiftDefsData} isAdmin={true} selfView={true} leadRoles={["理学療法士"]} reload={loadAll} shiftConfirmReqs={shiftConfirmReqs} timeTransferReqs={timeTransferReqs}/>;
        ...
        if(t==="有給管理"&&isLead) return <LeaveManager emps={emps.filter(e=>leadRolesList.includes(e.role))} leaves={leaves} lvReqs={lvReqs.filter(r=>emps.find(e=>String(e.id)===String(r.empId)&&leadRolesList.includes(e.role)))} shifts={shifts} shiftDefsData={shiftDefsData} reload={loadAll} canGrant={false}/>;
        if(t==="タイムカード"&&isLead) return <TimecardView emps={emps.filter(e=>leadRolesList.includes(e.role))} shifts={shifts} punches={punches} otReqs={otReqs} lvReqs={lvReqs} shiftDefsData={shiftDefsData} leadRoles={leadRolesList} reload={loadAll} onGotoShiftCalendar={()=>setTabName("シフト作成")} timeTransferReqs={timeTransferReqs} shiftConfirmReqs={shiftConfirmReqs}/>;
        return null;
      })()}
    </div>
  </div>;
